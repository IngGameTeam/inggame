...
class ComponentServiceDSL private constructor(
    ...
    override fun toString(): String {
        return "Component{type=${type.name}, name=$name, parents$parents${if (key === null) "" else ", key=$key"}${if (root === null) "" else ", root=$root"}}"
    }
...
fun ComponentServiceDSL.createComponentModule(): Module = this.let { cs ->
    module(createdAtStart = true) {
        ...
        single(named(cs.name), createdAtStart = true) {
            (if (cs.parents.isEmpty()) EmptyComponentServiceImp(cs.name)
                ...
                get(named(cs.parents.first())), cs.name
            )
